...
async def main():
    ...
    Account.ESTIMATED_FEE_MULTIPLIER = 2.0

    contract = Contract(address=address, abi=BRAAVOS_ABI, provider=account, cairo_version=0)

    cancel_call = contract.functions["cancel_deferred_remove_signer_req"].prepare(signer_id)
    tx = await account.execute(calls=cancel_call, auto_estimate=True)
